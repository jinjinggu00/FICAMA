...
class SoftDTW(torch.nn.Module):
    ...
    def attention_calc_distance_matrix(self, x, y):
        ...
        # print("x,y",x.shape,y.shape)  torch.Size([2000, 25, 256]) torch.Size([2000, 25, 256])
    ...
    def calc_distance_matrix(self, x, y):
        ...
        x = x.unsqueeze(2).expand(-1, n, m, d)  # torch.Size([250, 8, 8, 6400])
        y = y.unsqueeze(1).expand(-1, n, m, d)  # torch.Size([250, 8, 8, 6400])

        x = x.reshape(-1, d)  # torch.Size([16000, 6400])
        y = y.reshape(-1, d)  # torch.Size([16000, 6400])
        # print(x.shape, y.shape)#torch.Size([16000, 6400]) torch.Size([16000, 6400])
        x = x / (x.norm(dim=1, keepdim=True) + 1e-8)
        y = y / (y.norm(dim=1, keepdim=True) + 1e-8)

        # e_cos=torch.matmul(x,y.transpose(0,1))
        cos = x * y  # torch.Size([16000, 6400])
        e_cos = cos.sum(1)  # torch.Size([16000])
        # e_cos = torch.exp(sum_cos)
        e_cos = e_cos.view(-1, n, m)  # torch.Size([250, 8, 8])
        # dist = e_cos
